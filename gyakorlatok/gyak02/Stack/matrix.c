#include "matrix.h"
#define PI 3.14159265358979323846f //PI konstans
...
{
    if (ms->top >= MATRIX_STACK_SIZE - 1)
        return 0; // Stack overflow

    ms->top++;
    memcpy(ms->stack[ms->top], matrix, sizeof(float) * 9);
    return 1;
}
...
int pop_matrix(MatrixStack* ms, float matrix[3][3])
{
    ...
    memcpy(matrix, ms->stack[ms->top], sizeof(float) * 9);
    ms->top--;
    return 1;
}
