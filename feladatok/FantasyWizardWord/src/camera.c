...
void camera_init(Camera* cam, vec3 position, vec3 target, vec3 up) {
    cam->up = up;
    camera_reset(cam, target); // alapértékek beállítása a target alapján
}


void camera_update(Camera* cam, vec3 player_position) {
    // Számítsuk ki a kamerát a gömbkoordináták alapján
    ...
    cam->position = vec3_add(player_position, vec3_create(x, y, z));
...
void camera_orbit(Camera* cam, float delta_azimuth, float delta_elevation) {
    ...
    // Limitáljuk az elevációt, hogy ne forduljon át
...
void camera_reset(Camera* cam, vec3 player_position) {
    cam->distance = 5.0f;
    cam->azimuth = DEG2RAD * 180.0f;  // player mögött
